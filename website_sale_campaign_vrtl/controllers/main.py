    ...
    def campaign(self, campaign=None, **post):
        ...
        if campaign:
            ...
            if len(campaigns) > 0:
                ...
                return request.render('website_sale_campaign_vrtl.no_campaign', {'next_campaign_date': next_campaign_date})
            else:
                return request.render('website_sale_campaign_vrtl.no_campaign', {'next_campaign_date': None})
            ...
            if o.object_id._name == 'res.partner':
                s = {'supplier_url': '/producer/%s' % o.object_id.id, 'supplier_name': o.object_id.name,
                     'supplier_image': '/website/imagemagick/res.partner/image/%s/%s' % (
                         o.object_id.id, request.env.ref('website_sale_campaign_vrtl.img_supplier_nav').id)}
