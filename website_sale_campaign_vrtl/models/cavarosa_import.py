...
class CavarosaImport(models.TransientModel):
                ...
                if importf.customers:
                    ...
                    for row in f:
                        ...
                        if row[u'Typ'] == 'Betalningsinformation':
                            ...
                            raise ValidationError('Unknown type: %s' % row[u'Typ'])
    ...
    def find_uom(self, name):
        ...
        if not uom:
            raise ValidationError("Couldn't find a unit of measure for %s." % name)
        return uom
    ...
    def find_country(self, name):
        ...
        if not country and len(country) == 1:
            raise ValidationError("Couldn't find a match for country %s" % name)
        return country.id
