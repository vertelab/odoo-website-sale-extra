...
class ProductSnippet(http.Controller):
    ...
    def product_change(self, product=None, **kw):
        ...
        return {
            'id': product.id, 'name': product.name, 'image': product.image_medium,
    ...
    def get_products_by_partner(self, partner_id=None, **kw):
        ...
        products = request.env['res.partner'].search([
            ('id', '=', int(partner_id))]).product_ids.filtered(lambda product: product.website_published)
